...
**Verdict: NOT SHIPPED -- study finding.** Whole-window rejection helps EAC at
*low* outlier rates, but its aggregation breaks down once many windows are
corrupted, and plain **LSI is both simpler and more robust**. The shipped
robustness tool is the basis-agnostic robust image (`robust=True`): 1.17 on
either basis under scattered outliers, 1.41 on the Legendre basis vs 3.65 on
the block basis under a contiguous burst. The overlapping-window ensemble is
not part of `dtfit`.
...
| outlier % | stock EAC | **overlapping-window ensemble (study only)** | LSI | curve_fit |
|---|---|---|---|---|
...
## Why it did not ship (the mechanism)
