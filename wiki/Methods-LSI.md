...
[DSBI](Methods-DSB). It fits the raw `(x, y)` directly (no symbolic pre-fit)
...
image `S = Phi^T (w y)`, `G = Phi^T diag(w) Phi` of [the image](Methods-Image)
...
- **frequency seeded** from the data's FFT peak via
  [`fft_frequency_seed`](API-Fitting#fft_frequency_seed) before the solve,
...
- **Spectral-Galerkin projection.** A weighted least-squares match in the
  ...
  orthogonal-polynomial test functions, where [EAC](Methods-EAC) uses local
...
order machinery underlies [`suggest_models`](API-Models)). Because the
...
[EACFilter](Methods-Equal-Areas-Filter); for the most noise-robust batch fit
with few parameters, [EAC](Methods-EAC); at scale (streams, blocks, many
channels), [ImageStream](Methods-Scaling).
